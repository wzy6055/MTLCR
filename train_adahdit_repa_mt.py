...
from data.ISPRS_dataset import ISPRSDataset, convert_to_color
...
def get_cached_features(batch, feat_db, device='cuda'):
    ...
    for path in batch['filename']:
        file_name = os.path.basename(path)  # 从绝对路径取文件名
...
def main(args):
    ...
    # ISPRS data
    if 'Potsdam' in config.dataset.data_dir or 'Vaihingen' in config.dataset.data_dir:
    ...
    feats_db = np.load(config.repa.feat_path, allow_pickle=True).item()

    for epoch in range(config.optimization.epochs):
        engine.model.train()
        for batch in train_dataloader:
            zs = get_cached_features(batch, feats_db)
            batch['zs'] = zs
            with accelerator.accumulate(engine.model):

                # loss = engine(batch)
                denoising_loss_mean, proj_loss, seg_loss = engine(batch)
                denoising_loss_mean = denoising_loss_mean.mean()
                proj_loss_mean = proj_loss.mean()
                seg_loss_mean = seg_loss.mean()

                loss_mean = denoising_loss_mean + proj_loss_mean * config.optimization.proj_coeff + 1. * seg_loss_mean
            ...
            if (global_step == 1 or (global_step % config.logging.sampling_steps == 0 and global_step > 0)):
                ...
                for j, batch_val in enumerate(tqdm(val_dataloader, desc='Val')):
                    zs = get_cached_features(batch_val, feats_db)
                    batch_val['zs'] = zs
                    engine.test_step(batch_val)
                metrics = engine.avg_metrics.value()
                cknna_score = engine.avg_cknna.value()
                accelerator.log(metrics, step=global_step)
                accelerator.log(cknna_score, step=global_step)
                ...
                accelerator.log({"input": wandb.Image(array2grid(image_results['input'])),
                                 ...
                                 "mask_label": wandb.Image(convert_to_color(image_results['mask_label'][0])),
                                 "mask_pred": wandb.Image(convert_to_color(image_results['mask_pred'][0])),
                                 },
                                step=global_step)
                engine.model.train()
            ...
            logs = {
                ...
                "seg_loss": accelerator.gather(seg_loss_mean).mean().detach().item(),
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="config/seg/vaihingen_thick_adahdit_B2_xpvl_seg_200k.yml")
