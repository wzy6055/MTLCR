...
from models.adahdit import image_transformer
    ...
    for epoch in range(config.optimization.epochs):
        ...
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(engine.model):
                ...
                # loss = engine(batch)
                denoising_loss_mean, proj_loss = engine(batch)
                denoising_loss_mean = denoising_loss_mean.mean()
                proj_loss_mean = proj_loss.mean()

                loss_mean = denoising_loss_mean + proj_loss_mean * config.optimization.proj_coeff
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="config/ablation_repa/vaihingen_thick_adahdit_B2_epvl_repa_200k.yml")
